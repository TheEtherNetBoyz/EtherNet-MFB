...
static const GameCombo kCombos[] = {
    // Area Reload (L+R+Start+A), exclusive
    {
        ...
        false,
        [] {
            return getSettings().game.areaReload.getValue() &&
                   !speedrun::isActive();
        },
        [] { reload_area(); },
        0,
    ...
    // Move Link (L+R+Y), pass-through, non-exclusive
    {
        ...
        false,
        [] {
            return getSettings().game.enableMoveLinkCombo.getValue() &&
                   !speedrun::isActive();
        },
    ...
    // Teleport save (R+D-pad Up), consumes D-pad Up, exclusive
    {
        ...
        false,
        [] {
            return getSettings().game.enableTeleportCombo.getValue() &&
                   !speedrun::isActive() &&
                   (mDoCPd_c::getHold(PAD_1) & PAD_TRIGGER_L) == 0 &&
                   getPlayer() != nullptr;
        },
        [] {
            auto* p = getPlayer();
    ...
    // Teleport load (R+D-pad Down), consumes D-pad Down, exclusive
    {
        ...
        [] {
            return getSettings().game.enableTeleportCombo.getValue() &&
                   !speedrun::isActive() &&
                   (mDoCPd_c::getHold(PAD_1) & PAD_TRIGGER_L) == 0 &&
                   s_hasTeleportPos && getPlayer() != nullptr;
        },
        [] {
            auto* p = getPlayer();
    ...
    // Moon Jump (R+A, hold), continuous, pass-through, non-exclusive
    {
        ...
        false,
        [] {
            return getSettings().game.moonJump && !speedrun::isActive() &&
                   getPlayer() != nullptr;
        },
...
void processGameCombos() {
    if (!getSettings().game.enableMoveLinkCombo.getValue() || speedrun::isActive()) {
